...
ggsave("visuals/calories_vs_steps.png", plot =daily_activity_new , width = 7, height = 5)
...
sleep_calories_plot <- ggplot(activity_sleep_df,aes(x=Calories, y=TotalMinutesAsleep)) + geom_point(color="purple") +
...
ggsave("visuals/sleep_vs_calories.png", plot = sleep_calories_plot, width = 7, height = 5)
...
hourly_intensity_plot <- ggplot(hourly_intensity_mean, aes(x = Time, y = MeanTotalIntensity)) +
...
ggsave("visuals/hourly_intensity.png", plot = hourly_intensity_plot, width = 7, height = 5)

# Saveboxplot using png() and dev.off()
png("visuals/activity_intensity_categories.png", width = 700, height = 500)
boxplot(activity_sleep_df$LightlyActiveMinutes, 
        activity_sleep_df$FairlyActiveMinutes, 
        activity_sleep_df$VeryActiveMinutes, 
        col = c("lightblue", "yellow", "green"), 
        xlab = "Activity Intensity",
        ylab = "Minutes", 
        main = "Light, Fair, and Very Active Minutes", 
        names = c("Light","Fair","Very"))
dev.off()
