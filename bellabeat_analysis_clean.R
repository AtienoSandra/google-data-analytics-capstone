...
setwd("C:/Users/Hp/OneDrive/Desktop/SANDY/bellabeatProject")

# Import datasets from local data folder
...
ggsave("visuals/activity_histogram.png", width = 7, height = 5)
...
ggsave("visuals/activity_vs_calories.png", width = 7, height = 5)
...
# --- Plot 1: Daily Activity (Steps vs Calories) ---
activity_calories_plot <- ggplot(daily_activity_new, aes(x = TotalSteps, y = Calories))+
  geom_point(color = "purple", alpha = 0.6)+
  geom_smooth(method = "lm", color= "black")+
  labs(title = "Daily Activity: Steps vs Calories", x = "Total Steps", y = "Calories Burned")+
  annotate("text", x = Inf, y = Inf, label = cor_cal_steps_text, hjust = 1.1, vjust = 1.5, size = 4, color = "red")
ggsave("visuals/daily_activity.png", plot = activity_calories_plot, width = 7, height = 5)

# --- Plot 2: Sleep Patterns (Minutes Asleep vs Time in Bed) ---
sleep_plot <- ggplot(sleep_day_new, aes(x = TotalTimeInBed, y = TotalMinutesAsleep)) +
  geom_point(color = "darkgreen", alpha = 0.6) +
  geom_smooth(method = "lm", se = FALSE, color = "orange") +
  labs(title = "Sleep Patterns: Minutes Asleep vs Time in Bed", x = "Time in Bed (minutes)", y = "Minutes Asleep")

ggsave("visuals/sleep_patterns.png", plot = sleep_plot, width = 7, height = 5)

# --- Plot 3: Activity vs Sleep (Steps vs Minutes Asleep) ---
activity_sleep_plot <- ggplot(activity_sleep_df, aes(x = TotalSteps, y = TotalMinutesAsleep)) +
  geom_point(alpha = 0.6, color = "purple") +
  geom_smooth(method = "lm", se = FALSE, color = "red") +
  labs(title = "Activity vs Sleep", x = "Total Steps", y = "Minutes Asleep")

ggsave("visuals/activity_vs_sleep.png", plot = activity_sleep_plot, width = 7, height = 5)
...
ggsave("visuals/sleep_vs_calories.png", plot = activity_sleep_plot, width = 7, height = 5)
